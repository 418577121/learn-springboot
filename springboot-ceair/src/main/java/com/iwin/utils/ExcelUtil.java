 ...
 * BeanCopy CustomException 属于自定义数据，属于可自定义依赖
...
public class ExcelUtil {
     ...
     */
    private static List[] readExcel(MultipartFile excel, Integer sheetNo, Class<?>[] rowModel) throws CustomException {
     ...
     * @param rowModel 实体类映射
     */
    public static List[] readExcel(MultipartFile excel, Class<?>... rowModel) throws CustomException {
        ExcelReader reader = getReader(excel);
        if (reader == null) {
            return new ArrayList[rowModel.length];
        }
     ...
     * getExtendsBeanList 主要是做Bean的属性拷贝 ，可以通过ExcelReader中添加的数据集直接获取
     */
    public static <T> List<T> readFirstSheetExcel(MultipartFile excel, Class<T> rowType) throws CustomException {
        ExcelReader reader = getReader(excel);
        if (reader == null) {
            return new ArrayList<>();
        }
     ...
     */
    public static <T> List readExcel(MultipartFile excel, Class<T> rowModel, int sheetNo) throws CustomException {
    ...
    public static <T> void writeExcel(HttpServletResponse response, List<T> list,
                                      String fileName, String sheetName, ExcelTypeEnum excelTypeEnum) throws CustomException {
     ...
     */
    public static <T> void writeExcel(HttpServletResponse response, List<T> list,
                                      String fileName, ExcelTypeEnum excelTypeEnum) throws CustomException {
        if (CollectionUtils.isEmpty(list)) {
            return;
        }
    ...
    public static void writeExcel(HttpServletResponse response, String fileName,
                                  ExcelTypeEnum excelTypeEnum, List... lists) throws CustomException {
     ...
     */
    private static OutputStream getOutputStream(String fileName, HttpServletResponse response, ExcelTypeEnum excelTypeEnum) throws CustomException {
        //创建本地文件
        ...
        try {
            ...
            throw new CustomException(CustomExceptionType.ADD_ERROR, "创建文件失败！");
        }
    }
     ...
     */
    public static ExcelReader getReader(MultipartFile excel) throws CustomException {
        String fileName = excel.getOriginalFilename();
        if (fileName == null) {
            throw new CustomException(CustomExceptionType.ADD_ERROR, "文件格式错误！");
        }
    ...
    public static List<String> get(MultipartFile excel, Class<?> rowType) throws CustomException {
        ExcelReader reader = getReader(excel);
        if (reader == null) {
            return new ArrayList<>();
        }
        ...
        readSheetList.add(readSheet);
        reader.read(readSheetList);
        List<String> objects = excelListener.get();
        return  objects;
    }
}
